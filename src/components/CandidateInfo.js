...
const CandidateInfo = ({
  ...
  const updateVote = () => {
    try {
      axios
        .post(
          `http://localhost:3001/api/participants/registeredparticipants/${urlPara.params.id}`,
          data
        )
        .then(response => {
  ...
  return (
    <div>
      ...
      <div className="container">
        ...
        <button className="VoteBtn homeBtn" onClick={updateVote}>
          Vote
        </button>{' '}
        <span id="updateStatus"></span> <br />
        <br />
      </div>
    </div>
  );
